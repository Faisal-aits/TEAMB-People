...
export function PTTMProvider({ children }) {
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const [p, ph, md, t, u, tk] = await Promise.all([
        api.get('/projects'),
        api.get('/pttm/phases'),
        api.get('/pttm/modules'),
        api.get('/pttm/teams'),
        api.get('/pttm/users'),
        api.get('/pttm/tasks')
      ]);
  ...
  const actions = useMemo(() => ({
    ...
    moduleName: id => modules.find(m => String(m.id) === String(id))?.name || '',
    ...
    saveModule: data => handle(async () => { const row = unwrap(await api.post('/pttm/modules', data)); await fetchAll(); return row; }, 'Module saved!'),
    updateModule: (id, data) => handle(async () => { const row = unwrap(await api.put(`/pttm/modules/${id}`, data)); await fetchAll(); return row; }, 'Module updated!'),
    deleteModule: id => handle(async () => { await api.delete(`/pttm/modules/${id}`); await fetchAll(); }, 'Module deleted'),
    ...
    submitForReview: id => handle(async () => { await api.post(`/pttm/tasks/${id}/submit-review`); await refreshTasks(); }, 'Task submitted for review!'),
    /** Team leader approves or rejects a task. action: 'approve' | 'reject' */
    reviewTask: (id, action, notes) => handle(async () => { await api.post(`/pttm/tasks/${id}/review`, { action, notes }); await refreshTasks(); }, action === 'approve' ? 'Task approved & completed!' : 'Task sent back for revision'),
    /** Fetch tasks pending review (optionally filtered by team_leader_id) */
    getPendingReviewTasks: async (teamLeaderId) => {
      const params = teamLeaderId ? `?team_leader_id=${teamLeaderId}` : '';
      return unwrap(await api.get(`/pttm/tasks/pending-review${params}`));
    },

    showToast,
    markSaved
  }), [fetchAll, handle, modules, phases, projects, refreshTasks, showToast, teams, users]);
