...
getEmployeeShiftForDate: async (employeeId, date) => {
    try {
        ...
        const defaultShift = await Shift.getEmployeeDefaultShift(employeeId);
        ...
        const systemDefault = await Shift.getDefaultShift();
    ...
    getEmployeeDefaultShift: async (employeeId) => {
        try {
            const query = `
                SELECT 
                    s.shift_id,
        ...
        try {
            ...
            const shiftsWithEmployees = await Promise.all(
                rows.map(async (shift) => {
                    const employees = await Shift.getEmployees(shift.shift_id);
                    return {
                        ...shift,
    ...
    getById: async (shiftId) => {
        try {
            ...
            const [rows] = await pool.execute(query, [shiftId]);
            return rows[0];
        } catch (error) {
    ...
    create: async (shiftData) => {
        ...
        try {
            ...
            const [shiftResult] = await connection.execute(
                `INSERT INTO tb_shifts (shift_name, check_in_time, check_out_time, is_default) 
                 VALUES (?, ?, ?, ?)`,
                [shiftData.shift_name, shiftData.check_in_time, shiftData.check_out_time, shiftData.is_default || false]
            );
            ...
            if (shiftData.is_default) {
                await connection.execute(
                    'UPDATE tb_shifts SET is_default = FALSE WHERE shift_id != ?',
                    [shiftId]
                );
                
                // Assign this shift to all employees without a specific shift assignment
                await Shift.assignDefaultShiftToEmployees(connection, shiftId);
            }
        ...
        try {
            ...
            const [updateResult] = await connection.execute(
                `UPDATE tb_shifts 
                 ...
                 WHERE shift_id = ?`,
                [shiftData.shift_name, shiftData.check_in_time, shiftData.check_out_time, shiftId]
            );
...
setAsDefault: async (shiftId) => {
    ...
    try {
        ...
        await connection.execute(
            'UPDATE tb_shifts SET is_default = FALSE'
        );

        // Set new default shift
        const [updateResult] = await connection.execute(
            'UPDATE tb_shifts SET is_default = TRUE WHERE shift_id = ?',
            [shiftId]
        );
...
assignDefaultShiftToEmployees: async (connection, shiftId) => {
    try {
        ...
        const [employees] = await connection.execute(
            `SELECT id FROM employee_details WHERE status = 'Active' OR status IS NULL`
        );
    ...
    delete: async (shiftId) => {
        ...
        try {
            ...
            const [shiftCheck] = await connection.execute(
                'SELECT is_default FROM tb_shifts WHERE shift_id = ?',
                [shiftId]
            );
            ...
            // Delete attendance records that reference this shift
            await connection.execute(
                'DELETE FROM tb_attendance WHERE shift_id = ?',
                [shiftId]
            );
            ...
            await connection.execute(
                'UPDATE employee_details SET default_shift_id = NULL WHERE default_shift_id = ?',
                [shiftId]
            );

            // Delete shift
            const [deleteResult] = await connection.execute(
                'DELETE FROM tb_shifts WHERE shift_id = ?',
                [shiftId]
            );
    ...
    getEmployees: async (shiftId) => {
        try {
            const query = `
                ...
                WHERE es.shift_id = ? AND es.assigned_date = CURDATE()
...
assignDefaultShiftForToday: async () => {
    try {
        const connection = await pool.getConnection();
        ...
        try {
            ...
            const [defaultShift] = await connection.execute(
                'SELECT shift_id FROM tb_shifts WHERE is_default = TRUE LIMIT 1'
            );
