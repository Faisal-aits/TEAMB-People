...
getAllTasks: async (req, res) => {
    try {
        ...
        let query = `
            ...
            LEFT JOIN teams tm ON t.team_id = tm.id AND tm.tenant_id = ?
            LEFT JOIN users u_assigned ON t.assigned_to_member = u_assigned.id
            LEFT JOIN users u_created ON t.created_by = u_created.id
            WHERE t.tenant_id = ?
        `;
        
        const values = [tenant_id, tenant_id, tenant_id];
        ...
        if (tasks.length > 0) {
            console.log('Sample task:', {
                id: tasks[0].id,
                title: tasks[0].title,
                assigned_to_member: tasks[0].assigned_to_member,
                assigned_to_name: tasks[0].assigned_to_name
            });
        }
